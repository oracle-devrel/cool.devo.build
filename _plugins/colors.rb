...
module Jekyll
  ...
  module Color
    ...
    # Returns true if Jekyll::Color supports the +feature+.
    #
    ...
    class << self
      ...
      ##   Jekyll::Color.coloring = STDOUT.isatty
      def coloring
        @coloring ||= true
      end
    ...
    ATTRIBUTES.each do |c, v|
      new_method = <<-EOSCRIPT
        def #{c}(string = nil)
          result = ''
          result << "\e[#{v}m" if Jekyll::Color.coloring?
          if block_given?
            result << yield
          ...
          result << "\e[0m" if Jekyll::Color.coloring?
          result
        end
      EOSCRIPT
    ...
    # Returns an array of all Jekyll::Color attributes as symbols.
    def attributes
      ATTRIBUTE_NAMES
    end
    extend self
